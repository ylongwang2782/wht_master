...
#define DWT_CB_DATA_RX_FLAG_RNG 0x1 // Ranging bit
     ...
     *
     * @brief This function sets the local data structure pointer to point to the element in the local array as given by
     * the index.
     *
     * input parameters
     ...
     *
     * NOTE: dwt_initialise() must be called prior to this function so that it can return a relevant value (stored in
     * OTP).
     *
     * input parameters
     *
     ...
     * e.g. when LDE is not used DWT_READ_OTP_PID     0x10 - read part ID from OTP DWT_READ_OTP_LID     0x20 - read lot
     * ID from OTP DWT_READ_OTP_BAT     0x40 - read ref voltage from OTP DWT_READ_OTP_TMP     0x80 - read ref
     * temperature from OTP output parameters
     *
     * returns DWT_SUCCESS for success, or DWT_ERROR for error
     */
     ...
     *
     * NOTE: These register settings are not preserved during SLEEP/DEEPSLEEP, thus they should be programmed again
     * after wake up
     *
     * input parameters
     * @param prf
     ...
     * output parameters
     *
     * returns DWT_SUCCESS for success, or DWT_ERROR for error (e.g. a delayed transmission will be cancelled if the
     ...
     *
     * input parameters
     * @param starttime - the TX/RX start time (the 32 bits should be the high 32 bits of the system time at which to
     * send the message, or at which to turn on the receiver)
     *
     * output parameters none
     *
     * no return value
     ...
     * @fn dwt_rxenable()
     *
     * @brief This call turns on the receiver, can be immediate or delayed (depending on the mode parameter). In the
     * case of a "late" error the receiver will only be turned on if the DWT_IDLE_ON_DLY_ERR is not set. The receiver
     * will stay turned on, listening to any messages until it either receives a good frame, an error (CRC, PHY header,
     * Reed Solomon) or  it times out (SFD, Preamble or Frame).
     *
     * input parameters
     ...
     * failed due to "late" error (DWT_START_RX_IMMEDIATE | DWT_NO_SYNC_PTRS) 4 used to re-enable RX without trying to
     * sync IC and host side buffer pointers, typically when performing manual RX re-enabling in double buffering mode
     *
     * returns DWT_SUCCESS for success, or DWT_ERROR for error (e.g. a delayed receive enable will be too far in the
     * future if delayed time has passed)
     */
     ...
     *
     * SNIFF mode is a low-power reception mode where the receiver is sequenced on and off instead of being on all the
     * time. The time spent in each state (on/off) is specified through the parameters below. See DW1000 User Manual
     ...
     * @param timeOn - duration of receiver ON phase, expressed in multiples of PAC size. The counter automatically adds
     * 1 PAC size to the value set. Min value that can be set is 1 (i.e. an ON time of 2 PAC size), max value is 15.
     ...
     * (after this "long sleep"), for a very short time to sample the air for a preamble sequence. This preamble
     * sampling "listening" phase is actually two reception phases separated by a "short sleep" time. See DW1000 User
     * Manual section "Low-Power Listening" for more details.
     *
     ...
     *
     * input parameters
     * @param  timeout - Preamble detection timeout, expressed in multiples of PAC size. The counter automatically adds
     * 1 PAC size to the value set. Min value that can be set is 1 (i.e. a timeout of 2 PAC size).
     *
     ...
     *
     * @brief calibrates the local oscillator as its frequency can vary between 7 and 13kHz depending on temp and
     * voltage
     *
     ...
     * @fn dwt_setcallbacks()
     *
     * @brief This function is used to register the different callbacks called when one of the corresponding event
     * occurs.
     *
     ...
     *        For all events, corresponding interrupts are cleared and necessary resets are performed. In addition, in
     the RXFCG case,
     ...
     *
     * NOTE:  In PC based system using (Cheetah or ARM) USB to SPI converter there can be no interrupts, however we
     still need something
     *        to take the place of it and operate in a polled way. In an embedded system this function should be
     ...
     * @fn dwt_isr_lplisten()
     *
     * @brief This is the DW1000's Interrupt Service Routine to use when low-power listening scheme is implemented. It
     * will only process/report the RXFCG event (through cbRxOk callback). It clears RXFCG interrupt and reads received
     * frame information and frame control before calling the callback.
     *
     ...
     * cleared
     *                  - if set to 2 the interrupts in the bitmask are forced to selected state - i.e. the mask is
     * written to the register directly.
     *
     * output parameters
     *
     * no return value
     ...
     *      DWT_FF_COORD_EN         0x002   behave as coordinator (can receive frames with no destination address (PAN
     * ID has to match)) DWT_FF_BEACON_EN        0x004   beacon frames allowed DWT_FF_DATA_EN          0x008   data
     * frames allowed DWT_FF_ACK_EN           0x010   ack frames allowed DWT_FF_MAC_EN           0x020   mac control
     * frames allowed DWT_FF_RSVD_EN          0x040   reserved frame types allowed
     *
     * output parameters
     *
     * no return value
     ...
     * @fn dwt_readaccdata()
     *
     * @brief This is used to read the data from the Accumulator buffer, from an offset location give by offset
     * parameter
     *
     * NOTE: Because of an internal memory access delay when reading the accumulator the first octet output is a dummy
     * octet that should be discarded. This is true no matter what sub-index the read begins at.
     *
     * input parameters
     * @param buffer - the buffer into which the data will be read
     * @param length - the length of data to read (in bytes)
...
#define HERTZ_TO_PPM_MULTIPLIER_CHAN_5 (-1.0e6 / 6489.6e6)
     ...
     *                  DWT_OPSET_DEFLT = 0x2 - load the default operational parameter set table (this is loaded from
     * reset)
     *
     * output parameters
     *
     ...
     * @fn dwt_getxtaltrim()
     *
     * @brief This function returns current value of XTAL trim. If this is called after dwt_initalise it will return the
     * OTP value if OTP value is non-zero or FS_XTALT_MIDRANGE if OTP value is zero (not programmed).
     *
     * input parameters
     *
     * output parameters
     ...
     * The dwt_initialise needs to be called before call to this to ensure pdw1000local->tempP contains the SAR_LTEMP
     * value from OTP.
     *
     * input parameters:
     ...
     * @fn dwt_calcpgcount()
     *
     * @brief this function calculates the value in the pulse generator counter register (PGC_STATUS) for a given
     * PG_DELAY This is used to take a reference measurement, and the value recorded as the reference is used to adjust
     * the bandwidth of the device when the temperature changes.
     *
     * input parameters:
     * @param pgdly - uint8 - the PG_DELAY to set (to control bandwidth), and to find the corresponding count value for
     * output parameters: None
     *
     * returns: (uint16) PGC_STATUS count value calculated from the provided PG_DELAY value - used as reference for
     * later bandwidth adjustments
     */
     ...
     *        a. check if sub index is used, if subindexing is used - set bit-6 to 1 to signify that the sub-index
     * address follows the register index byte b. set bit-7 (or with 0x80) for write operation c. if extended sub
     * address index is used (i.e. if index > 127) set bit-7 of the first sub-index byte following the first header byte
     *
...
#define dwt_read32bitreg(x) dwt_read32bitoffsetreg(x, 0)
    ...
    // ---------------------------------------------------------------------------
    //
    // NB: The purpose of the deca_mutex.c file is to provide for microprocessor interrupt enable/disable, this is used
    // for
     ...
     * @fn decamutexoff()
     *
     * @brief This function should re-enable interrupts, or at least restore their state as returned(&saved) by
     * decamutexon This is called at the end of a critical section
     *
     * Note: The body of this function is defined in deca_mutex.c and is platform specific
     *
     * input parameters:
