...
struct PingSession {
    ...
    std::unique_ptr<Message>
        originalMessage;    // Store original ping control message for response
    ...
    PingSession(uint32_t target, uint8_t mode, uint16_t total,
                uint16_t intervalMs, std::unique_ptr<Message> msg)
        : targetId(target),
          pingMode(mode),
          totalCount(total),
          currentCount(0),
...
struct PendingBackendResponse {
    ...
    std::unordered_set<uint32_t>
        pendingSlaves;    // Slaves that haven't responded
    std::unordered_map<uint32_t, uint8_t>
        slaveStatuses;     // Slave ID -> status (0=success, 1=error)
    ...
    bool isComplete() const { return pendingSlaves.empty(); }
